...
3. **辞書エコシステム (テンプレートエンジン)**
   ...
   - 内蔵辞書とユーザー独自の辞書をディープマージし、柔軟なカスタマイズが可能。
...
USB赤外線送信機 **AD00020P** を使用する場合、ハードウェアの仕様（不具合）により、一部のNEC方式の受信機が反応しないことがあります。

その際は、デバイス設定で `force_aeha_tx: true` を指定することで、内部的にNECフォーマットをAEHAフォーマットに偽装して送信し、この問題を回避できる可能性があります。
