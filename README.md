...
transmitters:
  tx_study:
    ...
    type: usb_ad00020p
    index: 0
    local_receivers: ["rx_study_usb"] # 無限ループ防止
...
USB赤外線送信機 **AD00020P** を使用する場合、一部のNEC方式の受信機が反応しないハードウェアの不具合があります。その際は、デバイス設定で `force_aeha_tx: true` を指定することで、内部的にAEHA形式として送信し回避できる可能性があります。
