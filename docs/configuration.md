# 詳細設定ガイド (Configuration Guide V2)

`IR-Trigger.yaml` を使用した送信機・受信機の管理、マルチ状態管理、および高度な設定方法について解説します。V2 では各コンポーネントが明確に分離されました。
...
- `type`: `usb_ad00020p` (旧 local_usb), `esphome`, `nature_remo`, `webhook`, `mock`
