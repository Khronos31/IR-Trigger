...
IR-Trigger はエッジデバイス（受信機）側で信号を受信し、Home Assistant へ通知する仕組みです。V2 アーキテクチャでは、受信機ごとに固有の Webhook ID を使用する RESTful な設計となっています。
...
```
- V2 では `receiver` フィールドの送信は不要です。Webhook ID（URL）によって送信元が識別されます。
